...
<aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform -translate-x-full bg-white border-r border-gray-200 sm:translate-x-0" aria-label="Sidebar">
   <div class="h-full px-3 pb-4 overflow-y-auto bg-white ">
      <ul class="space-y-2 font-medium">
         ...
         <li>
            <a href="/admin/dashboard" class="flex items-center p-2 text-gray-900 rounded-lg  {{($title === 'Arsindo Admin') ? 'bg-primary text-white font-bold' : 'bg-white' }}">
              <svg class="h-6 w-6 text-neutral-600 {{($title === 'Arsindo Admin') ? 'bg-primary text-white font-bold' : 'bg-white' }}"  width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">  <path stroke="none" d="M0 0h24v24H0z"/>  <circle cx="12" cy="13" r="2" />  <line x1="13.45" y1="11.55" x2="15.5" y2="9.5" />  <path d="M6.4 20a9 9 0 1 1 11.2 0Z" /></svg>
        ...
        <li>
            <a href="/admin/order/index" class="flex items-center p-2 text-gray-900 rounded-lg {{($title === 'Booking Arsindo' || $title === 'Update Booking') ? 'bg-primary text-white font-bold' : 'bg-white' }}">
              <svg class="h-6 w-6 text-neutral-600 {{($title === 'Booking Arsindo' || $title === 'Update Booking') ? 'bg-primary text-white font-bold' : 'bg-white' }}"  width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">  <path stroke="none" d="M0 0h24v24H0z"/>  <circle cx="9" cy="7" r="4" />  <path d="M3 21v-2a4 4 0 0 1 4 -4h4a4 4 0 0 1 4 4v2" />  <path d="M16 11l2 2l4 -4" /></svg>
                <span class="flex-1 ms-3 whitespace-nowrap">Order</span>
            </a>
        </li>

        <li>
            <a href="/admin/review/index" class="flex items-center p-2 text-gray-900 rounded-lg {{($title === 'Review Arsindo' || $title === 'Update Review') ? 'bg-primary text-white font-bold' : 'bg-white' }}">
               <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6 {{($title === 'Review Arsindo' || $title === 'Update Review') ? 'bg-primary text-white font-bold' : 'bg-white' }}" >
